...
(define ppt-stream-in-tree-order
  (let lup ((result empty-stream))
    (if (stream-empty? result)
        (lup (stream-cons '(3 4 5) result))
          (stream-cons
           (stream-first result)
           (lup
            (stream-rest (stream-append result (stream (multiply T1 (stream-first result)) (multiply T2 (stream-first result)) (multiply T3 (stream-first result))))))))))
...
;(define (pairs G H)
;  (let lup [(g G) (h H) (n 1) (cn 1)]
;    (if (= n 1)
;        (stream-cons (cons (stream-first g) (stream-first h)) (lup G (stream-rest h) (add1 cn) (add1 cn)))
;        (stream-cons (cons (stream-first g) (stream-first h)) (lup (stream-rest g) h (sub1 n) cn)))))
